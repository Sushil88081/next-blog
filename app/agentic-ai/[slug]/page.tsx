...
const CATEGORY_ID = "agentic-ai" as const;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const postUrl = `${siteUrl}/agentic-ai/${post.slug}`;
...
export default async function AgenticAiPostPage({ params }: PageProps) {
  const post = await getPostBySlug(params.slug, CATEGORY_ID);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="mb-4 text-sm">
        <Link href="/agentic-ai" className="text-primary-600 dark:text-primary-400 hover:underline">
          ← Agentic AI (series)
        </Link>
      </div>
      <script
      ...
      <header className="mb-8">
        <div className="flex items-center flex-wrap gap-3 mb-4">
          ...
          {post.chapter != null && (
            <span className="text-gray-500 dark:text-gray-400 text-sm">Chapter {post.chapter}</span>
          )}
          {post.date ? (
      ...
      <CommentSection postSlug={post.slug} postCategory={CATEGORY_ID} />
    </div>
  );
}
