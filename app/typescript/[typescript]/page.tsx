...
export async function generateMetadata({
  ...
  const post = await getPostBySlug(params.typescript, "typescript");
  ...
  const postUrl = `${siteUrl}/typescript/${post.slug}`;
  const postImage = post.image
    ? `${siteUrl}${post.image}`
    : `${siteUrl}/assets/images/typescript.png`;
...
export default async function TypeScriptPostPage({ params }: PageProps) {
  const post = await getPostBySlug(params.typescript, "typescript");
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      {/* Comment Section */}
      <CommentSection postSlug={post.slug} postCategory="typescript" />
    </div>
  );
}
