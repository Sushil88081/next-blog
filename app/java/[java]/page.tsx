...
export async function generateMetadata({
  ...
  const post = await getPostBySlug(params.java, "java");
  ...
  const postUrl = `${siteUrl}/java/${post.slug}`;
  const postImage = post.image
    ? `${siteUrl}${post.image}`
    : `${siteUrl}/assets/images/java.png`;
  ...
  return {
    ...
    openGraph: {
      ...
      description: post.description,
      ...(post.date
        ? { publishedTime: post.date, modifiedTime: post.date }
        : {}),
...
export default async function JavaPostPage({ params }: PageProps) {
  const post = await getPostBySlug(params.java, "java");
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <CommentSection postSlug={post.slug} postCategory="java" />
    </div>
  );
}
